...
// Handle registration
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['username-reg'])) {
    $username = htmlspecialchars($_POST['username-reg']);
    $email = htmlspecialchars($_POST['email-reg']);
    $password = password_hash($_POST['password-reg'], PASSWORD_BCRYPT);
...
// Handle login
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['username-login'])) {
    ...
    if (pg_num_rows($result) > 0) {
        ...
        if (password_verify($password, $user['password'])) {
            echo "<script>window.onload = function() { showLoginSuccessAlert(); };</script>";
        } else {
            echo "<script>window.onload = function() { showErrorAlert('Incorrect password.'); };</script>";
        }
    } else {
        echo "<script>window.onload = function() { showErrorAlert('User not found.'); };</script>";
    }
}
?><!DOCTYPE html><html>
<head>
...
<!-- Floating Alert for Login Success -->
<div id="loginSuccessAlert" class="alert alert-success alert-dismissible fade show position-fixed top-0 start-50 translate-middle-x mt-3 w-75" role="alert" style="display: none; z-index: 1055;">
...
<div id="errorAlert" class="alert alert-danger alert-dismissible fade show position-fixed top-0 start-50 translate-middle-x mt-3 w-75" role="alert" style="display: none; z-index: 1055;">
  Error: Unable to register or login.
</div>
...
<div class="container-main">
  ...
  <div class="form-box login">
    <form action="index.php" method="POST">
      ...
      <div class="input-box">
        <input type="password" class="form-control" placeholder="Password" name="password-login" required />
